...
@ReactModule(name = RNThreadModule.NAME)
public class RNThreadModule extends NativeRNThreadSpec implements LifecycleEventListener {
  ...
  public void stopThread(final double threadId) {
    final int nativeThreadId = (int) threadId;
    final JSThread thread = threads.get(nativeThreadId);
    if (thread == null) {
      Log.d(NAME, "Cannot stop thread - thread is null for id " + nativeThreadId);
      return;
    }
  ...
  @ReactMethod
  @Override
  public void postThreadMessage(double threadId, String message) {
    final int nativeThreadId = (int) threadId;
    JSThread thread = threads.get(nativeThreadId);
    if (thread == null) {
      Log.d(NAME, "Cannot post message to thread - thread is null for id " + nativeThreadId);
      return;
    }
