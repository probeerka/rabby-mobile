...
async function checkEIP7702Delegation(
  address: string,
  chains: CHAINS_ENUM[],
  // requestETHRpc: ReturnType<typeof apiProvider>['requestETHRpc'],
  ...
  const requestETHRpc = apiProvider.requestETHRpc;
...
export const useEIP7702ApprovalsQuery = ({
  ...
  const { value, loading, error } = useAsync(async () => {
    if (!isActive || !accountAddress || !apiProvider.requestETHRpc) {
      return [];
    }
      ...
      .map(
        (e, _, arr) =>
          ({
            ...
            alias: currentAccount.aliasName,
            list: arr,
    ...
    const revokeList: (() => Promise<void>)[] = chainList.map(
      chain => async () => {
        try {
          ...
          const _nonce = await getRecommendNonce({
...
const EIP7702ApprovalsContext =
  React.createContext<EIP7702ApprovalsContextValue>({
